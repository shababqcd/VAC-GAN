...
def GeneratorNetwork28x28(CONTROLDIM = 64, mini_batch_size = 16, NN = 64, name='Gen'):
    GenLinput = lasagne.layers.InputLayer(shape=(None, CONTROLDIM), name=name + 'Linput')
    GenL1 = lasagne.layers.DenseLayer(incoming=GenLinput, num_units=7 * 7 * NN,
                                      nonlinearity=lasagne.nonlinearities.identity,
                                      name=name + 'L1')
    GenL1Reshape = lasagne.layers.ReshapeLayer(incoming=GenL1, shape=(mini_batch_size, NN, 7, 7),
                                               name=name + 'L1Reshape')
    ...
    Gen_out_layer = lasagne.layers.Conv2DLayer(incoming=GenL7, num_filters=1, filter_size=(3, 3),
                                               pad='same', nonlinearity=lasagne.nonlinearities.identity,
                                               name=name + '_out_layer')
    return Gen_out_layer

def DiscriminatorNetwork28x28(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Disc'):

    DiscLinput = lasagne.layers.InputLayer(shape=(None, 1, 28, 28), name=name+'Linput')
    ...
    DiscL11 = lasagne.layers.Conv2DLayer(DiscL7MP, num_filters=3 * NN, filter_size=(3, 3), pad='same',
                                         nonlinearity=lasagne.nonlinearities.elu, name=name + 'L11')
    DiscL12 = lasagne.layers.Conv2DLayer(DiscL11, num_filters=3 * NN, filter_size=(3, 3), pad='same',
    ...
    DiscL13FC = lasagne.layers.DenseLayer(DiscL12, num_units=CONTROLDIM, nonlinearity=lasagne.nonlinearities.identity,
                                          name=name + 'L13FC')
    DiscL14 = lasagne.layers.DenseLayer(DiscL13FC, num_units=7 * 7 * NN, nonlinearity=lasagne.nonlinearities.identity,
                                        name=name + 'L14')
    DiscL14Reshape = lasagne.layers.ReshapeLayer(incoming=DiscL14, shape=(mini_batch_size, NN, 7, 7),
    ...
    Disc_out_layer = lasagne.layers.Conv2DLayer(incoming=DiscL20, num_filters=1, filter_size=(3, 3),
                                                 pad='same', nonlinearity=lasagne.nonlinearities.identity,
                                                 name=name+'_out_layer')
...
def GeneratorNetwork48x48(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Gen'):
    ...
    Gen_out_layer = lasagne.layers.Conv2DLayer(incoming=GenL9, num_filters=1, filter_size=(3, 3),
                                                pad='same', nonlinearity=lasagne.nonlinearities.identity,name=name+'_out_layer')
    return Gen_out_layer

def ClassifierNetwork28x28(name='Classifier'):
    networkBi = lasagne.layers.InputLayer(shape=(None, 1, 28, 28), name=name+'Linput')
    ...
    networkBOut = lasagne.layers.DenseLayer(lasagne.layers.dropout(netBL3), num_units=10,
                                            nonlinearity=lasagne.nonlinearities.softmax, name=name+'_out_layer')
    return networkBOut


def DiscriminatorNetwork48x48(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Disc'):

    DiscLinput = lasagne.layers.InputLayer(shape=(None, 1, 48, 48), name=name+'Linput')
    DiscL1 = lasagne.layers.Conv2DLayer(DiscLinput, num_filters=NN, filter_size=(3, 3), pad='same',
                                         nonlinearity=lasagne.nonlinearities.elu, name=name+'L1')
    DiscL2 = lasagne.layers.Conv2DLayer(DiscL1, num_filters=NN, filter_size=(3, 3), pad='same',
                                         nonlinearity=lasagne.nonlinearities.elu, name=name+'L2')
    ...
    Disc_out_layer = lasagne.layers.Conv2DLayer(incoming=DiscL22, num_filters=1, filter_size=(3, 3),
                                                 pad='same', nonlinearity=lasagne.nonlinearities.identity,
                                                 name=name+'_out_layer')
    return Disc_out_layer
...
def ClassifierNetwork48x48(name='Classifier'):
    networkBi = lasagne.layers.InputLayer(shape=(None, 1, 48, 48), name=name+'Linput')
    netBL1 = lasagne.layers.Conv2DLayer(networkBi, num_filters=16, filter_size=(3, 3), name=name+'L1')
    netBL1MP = lasagne.layers.MaxPool2DLayer(netBL1, pool_size=(2, 2), name=name+'L1MP')
    netBL2 = lasagne.layers.Conv2DLayer(netBL1MP, num_filters=8, filter_size=(3, 3), name=name+'L2')
    netBL2MP = lasagne.layers.MaxPool2DLayer(netBL2, pool_size=(2, 2), name=name+'L2MP')
...
def DiscriminatorNetwork64x64(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Disc'):
    ...
    DiscLinput = lasagne.layers.InputLayer(shape=(None, 1, 64, 64), name=name+'Linput')
    DiscL1 = lasagne.layers.Conv2DLayer(DiscLinput, num_filters=NN, filter_size=(3, 3), pad='same',
                                         nonlinearity=lasagne.nonlinearities.elu, name=name+'L1')
    DiscL2 = lasagne.layers.Conv2DLayer(DiscL1, num_filters=NN, filter_size=(3, 3), pad='same',
                                         nonlinearity=lasagne.nonlinearities.elu, name=name+'L2')
...
def ClassifierNetwork64x64(name='Classifier'):
    networkBi = lasagne.layers.InputLayer(shape=(None, 1, 64, 64), name=name+'Linput')
    netBL1 = lasagne.layers.Conv2DLayer(networkBi, num_filters=16, filter_size=(3, 3), name=name+'L1')
    netBL1MP = lasagne.layers.MaxPool2DLayer(netBL1, pool_size=(2, 2), name=name+'L1MP')
    netBL2 = lasagne.layers.Conv2DLayer(netBL1MP, num_filters=8, filter_size=(3, 3), name=name+'L2')
    netBL2MP = lasagne.layers.MaxPool2DLayer(netBL2, pool_size=(2, 2), name=name+'L2MP')
...
def GeneratorNetwork96x96(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Gen'):
    ...
    GenL11 = lasagne.layers.Conv2DLayer(GenL10, num_filters=NN, filter_size=(3, 3), pad='same',
                                        nonlinearity=lasagne.nonlinearities.elu, name=name+'L11')
    Gen_out_layer = lasagne.layers.Conv2DLayer(incoming=GenL11, num_filters=1, filter_size=(3, 3),
                                                pad='same', nonlinearity=lasagne.nonlinearities.identity,name=name+'_out_layer')
    return Gen_out_layer


def DiscriminatorNetwork96x96(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Disc'):
    DiscLinput = lasagne.layers.InputLayer(shape=(None, 1, 96, 96), name=name + 'Linput')
    DiscL1 = lasagne.layers.Conv2DLayer(DiscLinput, num_filters=NN, filter_size=(3, 3), pad='same',
                                        nonlinearity=lasagne.nonlinearities.elu, name=name + 'L1')
    DiscL2 = lasagne.layers.Conv2DLayer(DiscL1, num_filters=NN, filter_size=(3, 3), pad='same',
                                        nonlinearity=lasagne.nonlinearities.elu, name=name + 'L2')
    ...
    Disc_out_layer = lasagne.layers.Conv2DLayer(incoming=DiscL27, num_filters=1, filter_size=(3, 3),
                                                pad='same', nonlinearity=lasagne.nonlinearities.identity,
                                                name=name + '_out_layer')
    return Disc_out_layer

def ClassifierNetwork96x96(name='Classifier'):
    networkBi = lasagne.layers.InputLayer(shape=(None, 1, 96, 96), name=name+'Linput')
    netBL1 = lasagne.layers.Conv2DLayer(networkBi, num_filters=16, filter_size=(3, 3), name=name+'L1')
...
def DiscriminatorNetwork128x128(CONTROLDIM = 64,mini_batch_size = 16,NN = 64,name = 'Disc'):
    DiscLinput = lasagne.layers.InputLayer(shape=(None, 1, 128, 128), name=name + 'Linput')
    DiscL1 = lasagne.layers.Conv2DLayer(DiscLinput, num_filters=NN, filter_size=(3, 3), pad='same',
                                        nonlinearity=lasagne.nonlinearities.elu, name=name + 'L1')
    DiscL2 = lasagne.layers.Conv2DLayer(DiscL1, num_filters=NN, filter_size=(3, 3), pad='same',
                                        nonlinearity=lasagne.nonlinearities.elu, name=name + 'L2')
...
def ClassifierNetwork128x128(name='Classifier'):
    networkBi = lasagne.layers.InputLayer(shape=(None, 1, 128, 128), name=name+'Linput')
    netBL1 = lasagne.layers.Conv2DLayer(networkBi, num_filters=16, filter_size=(3, 3), name=name+'L1')
    netBL1MP = lasagne.layers.MaxPool2DLayer(netBL1, pool_size=(3, 3), name=name+'L1MP')
    netBL2 = lasagne.layers.Conv2DLayer(netBL1MP, num_filters=8, filter_size=(3, 3), name=name+'L2')
    netBL2MP = lasagne.layers.MaxPool2DLayer(netBL2, pool_size=(3, 3), name=name+'L2MP')
