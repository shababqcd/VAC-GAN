...
def build_generator(input_var=None):
    ...
    # input: 100dim
    layer = InputLayer(shape=(None, CONTROLDIM), input_var=input_var)
...
train_fn = theano.function([noise_var, input_var,target_varB],
                           [(real_out > .5).mean(),
                            (fake_out < .5).mean(),NetB_loss_T,lasagne.objectives.binary_crossentropy(fake_out, 1).mean()],
                           updates=updates)
...
for epoch in range(num_epochs):
    ...
    for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=True):
        ...
        targetsNum = np.concatenate([targetsNum, targetsNum])
    ...
    samples = gen_fn(lasagne.utils.floatX(np.random.rand(42, CONTROLDIM)))
    try:
        ...
        plt.imsave('mnist_samples.png',
                   (samples.reshape(6, 7, 28, 28)
                       ...
                       .reshape(6 * 28, 7 * 28)),
                   cmap='gray')
    with open('netBestGASADiscriminatorSingleGenMNIST_DCGANAttempt5_10ClassesEpoch' + str(
                    epoch) + '.pickle', 'wb') as handle:
        print('saving the model Discriminator....')
        pickle.dump(discriminator, handle)

    with open('netBestGASAGeneratorSingleGenMNIST_DCGANAttempt5_10ClassesEpoch' + str(
                    epoch) + '.pickle', 'wb') as handle:
